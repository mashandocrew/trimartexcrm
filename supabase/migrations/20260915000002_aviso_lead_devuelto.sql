-- Trimartex CRM — avisarle a Tristán cuando Joaquín le devuelve un lead de
-- Leads Tristán a Cartera o a su Gestión Privada.
--
-- El lead de `leads` se borra al devolverlo (y notificaciones.lead_id es ON
-- DELETE CASCADE), así que el aviso apunta al lead nuevo de la Gestión Privada
-- vía lead_privado_id; si volvió a Cartera no lleva lead.

alter table public.notificaciones drop constraint notificaciones_tipo_check;
alter table public.notificaciones add constraint notificaciones_tipo_check
  check (tipo = any (array[
    'seguimiento_vencido', 'sin_movimiento', 'recordatorio_manual',
    'recordatorio_automatico', 'lead_compartido', 'lead_devuelto'
  ]));
